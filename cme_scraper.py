...
def _select_btc(frame):
    """Navigate product selector: Cryptocurrencies → Bitcoin (BTC|BTC)."""
    _close_popup(frame, "ctl11_ucProductSelectorPopup_pnlProductSelectorPopup")
    _close_popup(frame, "ctl11_ucProductOnlyPopup_pnlProductSelectorPopup")
    ...
    frame.evaluate("""
        () => {
            for (const a of document.querySelectorAll(".groups a"))
                if (a.innerText.trim().toUpperCase().includes("CRYPTO")) {
                    a.dispatchEvent(new MouseEvent("mouseover",{bubbles:true})); a.click(); return;
                }
        }
    """)
    time.sleep(0.5)
    frame.evaluate("""
        () => {
            for (const a of document.querySelectorAll(".families a"))
                if (a.innerText.trim().toUpperCase().includes("CRYPTO")) { a.click(); return; }
        }
    """)
    time.sleep(0.5)
    ...
    name = frame.evaluate("""
        () => {
            for (const p of document.querySelectorAll(".products a"))
                if (p.innerText.includes("Bitcoin")) { p.click(); return p.innerText.trim(); }
            return null;
        }
    """)
    if not name:
        raise RuntimeError("Bitcoin (BTC|BTC) not found in product selector")
    print(f"  [Product] {name}")
    time.sleep(5)
    return name


def _select_nearest_weekly(frame, skip=0):
    """Open expiry selector, pick the nearest weekly. skip=N skips N nearest. Returns expiry info dict."""
    ...
    if not pos:
        pos = sorted(exps, key=lambda x: -x["dte"])
    idx = min(skip, len(pos) - 1)
    nearest = pos[idx]
...
def _run_oi_heatmap(ctx):
    ...
    _select_gold(frame)
    # Re-acquire frame in case product switch reloaded the iframe
    time.sleep(3)
    frame = _get_frame(page)
    frame.wait_for_load_state("domcontentloaded", timeout=15000)
    time.sleep(2)
...
def _run_btc_vol2vol(ctx):
    """Extract Vol2Vol data for Bitcoin (BTC|BTC). Returns dict."""
    page = next((pg for pg in ctx.pages if "vol2vol" in pg.url.lower()), None)
    if not page:
        page = ctx.new_page()
        page.goto(CME_VOL2VOL_URL, wait_until="domcontentloaded", timeout=30000)
...
def _run_btc_oi_heatmap(ctx):
    """Extract OI heatmap data for Bitcoin (BTC|BTC). Returns dict."""
    page = next((pg for pg in ctx.pages if "heatmap" in pg.url.lower()), None)
    if not page:
        page = ctx.new_page()
        page.goto(CME_HEATMAP_URL, wait_until="domcontentloaded", timeout=30000)
    ...
    # BTC near-term weekly options typically have 0 OI — use HasOI QuikPick to
    # auto-select the nearest BTC expiry that actually has Open Interest data.
    BTC_HASOI_TRIGGER = "MainContent_ucViewControl_IntegratedVOIHeatMap_ucExpirationsToolbar_ucQuikPick_ucTrigger_lnkTrigger"
    BTC_HASOI_BTN     = "MainContent_ucViewControl_IntegratedVOIHeatMap_ucExpirationsToolbar_ucQuikPick_lbHasOI"

    print(f"\n[BTC OI Heatmap] {page.url}")
    frame = _get_frame(page)
    frame.wait_for_load_state("domcontentloaded", timeout=15000)
    time.sleep(2)

    _select_btc(frame)
    # Re-acquire frame — BTC product change reloads the OI heatmap iframe
    time.sleep(3)
    frame = _get_frame(page)
    frame.wait_for_load_state("domcontentloaded", timeout=15000)
    time.sleep(2)
    ...
    # Select nearest expiry with OI > 0 via QuikPick
    print("  [Expiry]  Selecting nearest BTC expiry with OI > 0...")
    frame.evaluate(f'() => {{ const b=document.getElementById("{BTC_HASOI_TRIGGER}"); if(b) b.click(); }}')
    time.sleep(0.5)
    frame.evaluate(f'() => {{ const b=document.getElementById("{BTC_HASOI_BTN}"); if(b) b.click(); }}')
    time.sleep(6)

    exp_symbol = frame.evaluate('''
        () => {
            const btn = document.getElementById("ctl00_ucSelector_hlExpiration");
            return (btn?.innerText || "").replace(/EXPIRATION[:\\s\\xa0]+/gi,"").trim();
        }
    ''')
    print(f"  [Expiry]  {exp_symbol}")
    nearest = {"symbol": exp_symbol, "expDate": exp_symbol, "dte": "—"}
    ...
    rows = raw["rows"]
    if not rows:
        raise RuntimeError("No BTC OI data found")
...
def _write_cme_daily(v2v, oi, cot, spot, basis, adj, btc_v2v=None, btc_oi=None):
    ...
    if btc_v2v or btc_oi:
        ...
        if btc_v2v:
            lines += [
                ...
                f"| Field        | Value |",
                f"|--------------|-------|",
                f"| Symbol       | {btc_v2v['expiry_symbol']} |",
                f"| Exp Date     | {btc_v2v['expiry_date']} |",
                f"| DTE          | {btc_v2v['expiry_dte']} |",
                f"| Put Volume   | {btc_v2v['put_volume']:,} |" if btc_v2v['put_volume'] else "| Put Volume   | N/A |",
                f"| Call Volume  | {btc_v2v['call_volume']:,} |" if btc_v2v['call_volume'] else "| Call Volume  | N/A |",
                f"| Volatility   | {btc_v2v['volatility']}% |" if btc_v2v['volatility'] else "| Volatility   | N/A |",
                f"| BTC Futures  | {btc_v2v['future_price']:,.0f} |" if btc_v2v['future_price'] else "| BTC Futures  | N/A |",
                f"",
            ]
        if btc_oi:
            lines += [
                f"### OI Heatmap — Key Levels",
                f"| Level      | Strike | OI |",
                f"|------------|--------|----|",
                f"| Put Wall   | {btc_oi['put_wall']:,.0f} | {btc_oi['put_wall_oi']:,} |",
                f"| Call Wall  | {btc_oi['call_wall']:,.0f} | {btc_oi['call_wall_oi']:,} |",
                f"| Max Pain   | {btc_oi['max_pain']:,.0f} | — |",
                f"| Data Date  | {btc_oi['data_date']} | |",
                f"",
            ]
...
def fetch_btc_vol2vol():
    with sync_playwright() as pw:
        ctx = pw.chromium.connect_over_cdp("http://localhost:9222").contexts[0]
        d = _run_btc_vol2vol(ctx)
        print()
        print("=" * 52)
        print("  VOL2VOL  —  Bitcoin (BTC|BTC)")
        print(f"  Expiry      : {d['expiry_symbol']} ({d['expiry_date']}, DTE={d['expiry_dte']})")
        print(f"  Put Volume  : {d['put_volume']:,}"  if d['put_volume']  else "  Put Volume  : N/A")
        print(f"  Call Volume : {d['call_volume']:,}" if d['call_volume'] else "  Call Volume : N/A")
        print(f"  Volatility  : {d['volatility']}%"  if d['volatility']  else "  Volatility  : N/A")
        print(f"  Futures     : {d['future_price']:,.0f}" if d['future_price'] else "  Futures     : N/A")
        print("=" * 52)


def fetch_btc_oi_heatmap():
    with sync_playwright() as pw:
        ctx = pw.chromium.connect_over_cdp("http://localhost:9222").contexts[0]
        d = _run_btc_oi_heatmap(ctx)
        print()
        print("=" * 62)
        print("  OI HEATMAP  —  Bitcoin (BTC|BTC)")
        print(f"  Expiry      : {d['expiry_symbol']} ({d['expiry_date']}, DTE={d['expiry_dte']})")
        print(f"  Data Date   : {d['data_date']}")
        print(f"  Put Wall    : {d['put_wall']:,.0f}  (OI={d['put_wall_oi']:,})")
        print(f"  Call Wall   : {d['call_wall']:,.0f}  (OI={d['call_wall_oi']:,})")
        print(f"  Max Pain    : {d['max_pain']:,.0f}")
        print()
        print(f"  {'Strike':>8}  {'Call OI':>8}  {'Put OI':>8}")
        print(f"  {'--------':>8}  {'--------':>8}  {'--------':>8}")
        for r in sorted(d["rows"], key=lambda x: x["strike"]):
            print(f"  {r['strike']:>8,.0f}  {r['callOI']:>8,}  {r['putOI']:>8,}")
        print("=" * 62)
...
def fetch_all():
    ...
    with sync_playwright() as pw:
        ...
        btc_v2v = btc_oi = None
        try:
            btc_v2v = _run_btc_vol2vol(ctx)
            btc_oi  = _run_btc_oi_heatmap(ctx)
    ...
    # Write markdown
    path = _write_cme_daily(v2v, oi, cot, spot, basis, adjusted, btc_v2v, btc_oi)
...
if __name__ == "__main__":
    ...
    elif cmd == "btc":        fetch_btc_vol2vol()
    elif cmd == "btc_oi":     fetch_btc_oi_heatmap()
    else:                     fetch_vol2vol()
