...
Lot Size: Midas ประเมินเองตาม Setup และขนาดพอร์ต
    ทุนน้อย ($50-200)  → Risk สูงได้ เป้าหมายปั้นพอร์ต
    ทุนมากขึ้น         → ลด Risk ลงตามสัดส่วน
    Setup แข็งแกร่งมาก → ALL IN ได้
```
...
```
หลักการ:
ทุนน้อย ($50-200)   → Risk สูงได้ เป้าหมายปั้นพอร์ต 200-400%
ทุนปานกลาง ($200+)  → Risk ลดลงตามสัดส่วน
ทุนมาก ($1000+)     → Risk หลักสิบ % ต่อไม้

ALL IN ได้เมื่อ:
- ผ่านทุกขั้นตอนใน Core Setup
- Confidence = HIGH
- CME ยืนยันทิศทางเดียวกัน
- SL แคบ (300-800 pip สำหรับ GOLD)
...
🔴 Priority 2: Invalidation Rules
   Setup ที่ ARMED อยู่ต้องถูกยกเลิกและรีเซ็ต SCANNING เมื่อ:
   - OB ที่เป็น Reference ถูกทะลุ (ราคาปิดพ้น OB)
   - Setup อายุเกิน 2 ชั่วโมงนับจาก SCANNING
   - มีข่าวแดง HIGH Impact ใน 30 นาทีข้างหน้า
   - H1 swing_trend เปลี่ยนทิศ (Bias เดิมไม่ valid แล้ว)

🟡 Priority 3: Morning Brief (3 ครั้ง/วัน)
   สรุปภาพรวมตลาดก่อนเข้าแต่ละ Session ส่งผ่าน Telegram:
   - 08:00 BKK → Pre-Asia Brief
   - 14:00 BKK → Pre-London Brief
   - 19:00 BKK → Pre-NY Brief
   ใช้ข้อมูลจาก Hermes + SMC HTF เป็น Input
