...
## 16. CURRENT STATUS (ณ 24 พฤษภาคม 2026 — v3.6)
...
✅ Telegram Two-Way — รับคำสั่งจาก Telegram ได้ (main.py Polling Thread)
   - /status  → แสดงสถานะบอทและ Open Positions ทั้งหมด
   - /pause   → หยุดสแกน (MONITORING ยังทำงานต่อ ไม่กระทบไม้ที่เปิดอยู่)
   - /resume  → เปิดสแกนกลับมา
   - /close [ticket] → ปิดไม้ตาม Ticket ทันที (close_position_by_ticket)
   - /report  → สรุปผลการเทรดวันนี้ (Win/Loss/Profit รวม)
   - /help    → แสดงคำสั่งทั้งหมด
   IS_PAUSED Global: pauses SCANNING/WATCHING/PRE_ENTRY/EXECUTE; MONITORING ทำงานตลอด
```
...
*Requirements version 3.6 | อัปเดต: 2026-05-24*
